...
%	The cubes have size 6 x 6 x 6 voxels, and therefore the template can start at different off set points max 3 voxels from the borders.
...
%	- bl_ind.m files for each off set --> array where each step of n³ indices = 1 cube.
%
...
delete('all_dirs_6vox.txt');
fid1=fopen('all_dirs_6vox.txt', 'a');

% Loop through x, y and z dimensions to make sure to cover all possible combinations of [x, y, z] starting coordinates: Max +/-3 from the border
for indx=1:3
	...
	for indy=1:3
		...
		for indz=1:3
			...
			mkdir(strcat( char([int2str(indx), int2str(indy), int2str(indz)] ), '/data' ));
