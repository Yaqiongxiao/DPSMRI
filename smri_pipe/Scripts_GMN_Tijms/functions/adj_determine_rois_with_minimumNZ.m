function [nz, nan_count, off_set] = adj_determine_rois_with_minimumNZ(CN_a2, bl_dir, n, Sa)
...
% adjusted for use on the GRID 2016-09-12 by Ellen Dicks
...
% Store current directory -> subject's directory
tempd1=pwd;
...
for im2=1:numimages2
	tempd2=char(CN_a2(im2));		% bl_dir subdir
	t=strcat(tempd2,'/data/bl_ind.m');
	disp(['bl_dir subdir/data/bl_ind.m = ' t])
	t2=strcat(tempd1,'/bind.m');		% subj results dir / current dir
	disp(['sub dir/bind.m = ' t2])
	...
	disp(['deleting sub dir/bind.m = ' t2])
	delete(t2);
	fid=fopen(t2, 'a');
...
t=strcat(tempd2,'/data/bl_ind.m');
t2=strcat(tempd1,'/bind.m');	
%t2=strcat(tempd1,'/data/bind.m');	% subject's dir
