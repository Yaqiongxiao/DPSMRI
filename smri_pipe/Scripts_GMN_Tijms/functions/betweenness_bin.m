function [BC L]=betweenness_bin(G)
...
G=single(G);	% Make sure that the type is single otherwise this doesn work

n=length(G);                %number of nodes
I=(eye(n)~=0);                %logical identity matrix
d=single(1);                %path length
...
DP=single(zeros(n));            	%vertex on vertex dependency
